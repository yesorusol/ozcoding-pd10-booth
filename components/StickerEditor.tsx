                ...
                {BACKGROUND_PATTERNS.map((p) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        onBackgroundChange({ kind: "pattern", patternId: p.id })
                      }
                      aria-label={`${p.label} 배경`}
                      aria-pressed={isSelected}
                      className={`relative aspect-square overflow-hidden rounded border-2 transition active:scale-95 ${
                        isSelected
                          ? "border-cabinet-frame ring-2 ring-marquee-yellow"
                          : "border-cabinet-frame/30"
                      }`}
                      style={{
                ...
                {BACKGROUND_COLORS.map((c) => {
                  ...
                  return (
                    <button
                      ...
                      aria-pressed={isSelected}
                      className={`aspect-square rounded-full border-2 transition active:scale-95 ${
                        isSelected
                          ? "border-cabinet-frame ring-2 ring-marquee-yellow"
                          : "border-cabinet-frame/30"
                      }`}
          ...
          <div className="mb-4 grid grid-cols-3 gap-2">
            {CHARACTER_STICKERS.map((asset) => (
              <button
                ...
                className="flex aspect-square items-center justify-center overflow-hidden rounded border border-cabinet-frame/40 bg-white p-1 transition active:scale-95"
              >
          ...
          <div className="grid grid-cols-4 gap-2">
            {EMOJI_STICKERS.map((emoji) => (
              <button
                ...
                className="flex aspect-square items-center justify-center rounded border border-cabinet-frame/40 bg-white text-3xl transition active:scale-95"
              >
                {emoji.emoji}
              </button>
            ))}
