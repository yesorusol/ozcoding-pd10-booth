...
describe.each(NORMAL_FRAME_IDS)("getNormalCellRects(%s)", (frameId) => {
  const rects = getNormalCellRects(frameId);

  it("declares exactly 4 cells (one per polaroid)", () => {
    expect(rects).toHaveLength(4);
  });

  it("each cell stays within the sheet bounds (axis-aligned)", () => {
...
describe("parseNormalFrameId", () => {
  it("resolves the exact literal 'green'", () => {
    expect(parseNormalFrameId("green")).toBe("green");
  });

  it("falls back to 'clover' for missing or unknown values", () => {
    expect(parseNormalFrameId("clover")).toBe("clover");
    expect(parseNormalFrameId(null)).toBe("clover");
    expect(parseNormalFrameId(undefined)).toBe("clover");
    expect(parseNormalFrameId("Green")).toBe("clover");
    expect(parseNormalFrameId("나비")).toBe("clover");
  });
});
...
describe("cellCenter", () => {
  ...
  it("returns center for the first declared cell", () => {
    const r = getNormalCellRects("clover")[0];
